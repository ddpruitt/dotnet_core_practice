<Query Kind="Program">
  ...
  <Namespace>Raven.Client.Documents.Session.TimeSeries</Namespace>
...
void Main()
{
	using (IDocumentSession session = DocumentStoreHolder.Store.OpenSession("Test01"))
	{
		Company company = session.Load<Company>("companies/5-A");
		company.Dump();
	}
	using (IDocumentSession session = DocumentStoreHolder.Store.OpenSession("Test02"))
	{
		Company company = session.Load<Company>("companies/5-A");
		company.Dump();
	}
